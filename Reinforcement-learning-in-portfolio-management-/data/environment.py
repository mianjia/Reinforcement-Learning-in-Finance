...
class Environment:
    ...
    def get_data(self,start_time,end_time,features,window_length,market,codes):
        ...
        #为每一个资产生成数据 Generate data for each asset
        asset_dict=dict()#每一个资产的数据 Data for each asset
        for asset in codes:
            asset_data=data[data["code"]==asset].reindex(self.date_set).sort_index()
            #加入时间的并集，会产生缺失值
            #pd.to_datetime(self.date_list)
            # Adding a union of times will result in missing values
            ...
            #根据收盘价填充其他值  -  Fill in other values based on the closing price
            #***********************open as preclose*******************#
            #asset_data=asset_data.dropna(axis=0,how='any')
            asset_dict[str(asset)]=asset_data


        #开始生成tensor - Start generating tensor
        self.states=[]
        self.price_history=[]
        t =self.L+1
